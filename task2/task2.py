...
def calculate_node_relations(graph: Dict[int, List[int]], node: int) -> List[int]:
    ...
    row_1 = len(graph[node])
    row_2 = count_direct_ancestors(graph, node)
    row_3 = count_all_descendants(graph, node) - row_1
    row_4 = count_all_ancestors(graph, node) - row_2
    row_5 = count_near_nodes(graph, node)
